...
import { SIGNATURE_ITEMS, CURRENT_OFFERS, BRAND_TAGLINE } from './constants';
...
const App: React.FC = () => {
    ...
    try {
      ...
      if (error) {
        if (error.code === 'PGRST205') {
          setDbError("Database tables missing. Please run the SQL schema in Supabase.");
          return;
        }
        throw error;
      }
      ...
      if (data) {
        const serverSettings: AdminSettings = { 
          ...
          menuItems: (data.config.menuItems && data.config.menuItems.length > 0) ? data.config.menuItems : DEFAULT_SETTINGS.menuItems,
          offers: (data.config.offers && data.config.offers.length > 0) ? data.config.offers : DEFAULT_SETTINGS.offers,
        ...
        if (serverSettings.highlightTag === "Today's Pick" && serverSettings.highlightSetDate) {
        ...
        await supabase.from('settings').upsert([{ id: 'app_config', config: DEFAULT_SETTINGS }]);
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('member_updates')
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'members', filter: `member_id=eq.${user.memberId}` }, (payload) => {
  ...
  const handleRegister = async (name: string, phone: string, followedSocial: boolean) => {
  ...
  const updateAdminSettings = async (newSettings: AdminSettings) => {
    ...
    // Background Persist
    try {
      await supabase.from('settings').update({ config: newSettings }).eq('id', 'app_config');
    } catch (err) { console.error('Failed to save settings:', err); }
  };

  const handleCashierScanResult = async (memberId: string) => {
     try {
      const { data: member, error } = await supabase.from('members').select('*').eq('member_id', memberId).single();
      if (error || !member) return { success: false, message: "Member Not Found" };
      const newVisits = member.visits_in_cycle + 1;
      let newRewards = member.rewards_available;
      let finalVisits = newVisits;
      if (newVisits >= 5) { newRewards += 1; finalVisits = 0; }
      const { data: updatedData } = await supabase.from('members').update({
          points: member.points + 1, visits_in_cycle: finalVisits, rewards_available: newRewards, last_visit_date: new Date().toISOString()
        }).eq('member_id', memberId).select().single();
      const updatedMember: UserProfile = {
        firstName: updatedData.first_name, phone: updatedData.phone, points: updatedData.points, visitsInCycle: updatedData.visits_in_cycle,
        rewardsAvailable: updatedData.rewards_available, isRegistered: true, memberId: updatedData.member_id, lastVisitDate: updatedData.last_visit_date, isFollowingSocial: updatedData.is_following_social
      };
      if (user?.memberId === memberId) { setUser(updatedMember); localStorage.setItem('don_louis_user', JSON.stringify(updatedMember)); setShowScanSuccess(true); }
      return { success: true, member: updatedMember, rewardUnlocked: newVisits >= 5 };
  ...
  const renderScreen = () => {
    switch (currentScreen) {
      ...
      case AppScreen.HOME:
        return <HomeScreen user={user} adminSettings={adminSettings} onScan={() => {}} />;
      case AppScreen.MENU:
  ...
  const isFullPageMode = [
    AppScreen.SPLASH, AppScreen.JOIN, AppScreen.CASHIER_AUTH, 
    AppScreen.CASHIER_SCAN, AppScreen.ADMIN_AUTH, AppScreen.ADMIN_DASHBOARD
  ...
  return (
    <div className="relative min-h-screen bg-black overflow-hidden select-none">
      ...
      <AnimatePresence>
        {showScanSuccess && (
          <ScanSuccessModal onClose={() => setShowScanSuccess(false)} points={user?.points || 0} />
        )}
      </AnimatePresence>

      <AnimatePresence>
        {showInactivityModal && user && (
          <InactivityModal onClose={() => setShowInactivityModal(false)} userPoints={user.points} userName={user.firstName} />
        )}
      </AnimatePresence>
    </div>
  );
