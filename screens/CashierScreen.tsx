...
const CashierScreen: React.FC<CashierScreenProps> = ({
  ...
  const CORRECT_PIN = cashierPin || "1977";
  ...
  if (!isAuthenticated) {
    return (
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className="fixed inset-0 bg-[#030303] z-[300] flex flex-col items-center justify-center p-8"
      >
        <button
          onClick={onExit}
          ...
          {[0, 1, 2, 3].map((i) => (
            <div
              ...
              className={`w-2 h-2 rounded-full transition-all duration-300 ${
                pin.length > i ? "bg-white" : "bg-zinc-800"
              }`}
            />
          ))}
        </div>
        <div className="grid grid-cols-3 gap-x-8 gap-y-8">
          {["1", "2", "3", "4", "5", "6", "7", "8", "9"].map((num) => (
            <button
              ...
              className="w-16 h-16 rounded-full text-2xl font-light text-white active:bg-white/10 transition-colors font-serif"
            >
              {num}
            </button>
          ))}
          ...
          <button
            onClick={() => setPin("")}
            className="w-16 h-16 rounded-full flex items-center justify-center text-zinc-600 active:text-white"
          >
            <RefreshCcw size={20} />
          </button>
        </div>
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="fixed inset-0 bg-[#030303] z-[300] flex flex-col p-6"
    >
      <div className="flex justify-between items-center mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-600 font-serif">Cashier Scanner</p>
          </div>
        </div>
        <button
          onClick={onExit}
          className="w-10 h-10 rounded-full bg-zinc-900 flex items-center justify-center text-zinc-500 hover:text-white"
        >
          <X size={20} />
        </button>
      </div>
        ...
        <AnimatePresence mode="wait">
          {isProcessing ? (
            ...
            <motion.div
              ...
              className="w-full max-w-sm bg-[#101010] rounded-[2rem] p-10 text-center border border-white/5"
            >
              ...
              <p className="text-zinc-500 mb-8 text-[10px] uppercase tracking-widest">
                Verification Complete
              </p>

              {lastScanResult.reward && (
                <div className="mb-8 p-4 bg-[#BF953F]/10 border border-[#BF953F]/20 rounded-xl">
                  <p className="text-[#BF953F] font-bold text-[10px] uppercase tracking-widest">
                    Reward Unlocked
                  </p>
                </div>
              )}
              ...
              <button
                ...
                className="w-full bg-white text-black py-4 rounded-xl font-bold uppercase tracking-widest text-[10px]"
              >
                Next Member
              </button>
            </motion.div>
