...
const HomeScreen: React.FC<HomeScreenProps> = ({
  ...
  useEffect(() => {
    ...
    if (isOneAway) {
      setNotificationMessage({
        title: "Keep Going",
        body: "You are 1 visit away from your reward.",
      });
      triggerNotification();
      return;
    }
    ...
    if (!triggered && Math.random() < 0.2) {
      setNotificationMessage({
        title: "Philosophy",
        body: adminSettings.content.brandTagline || BRAND_TAGLINE,
      });
      triggerNotification();
    }
  ...
  const highlightedItem = useMemo(() => {
    return (
      adminSettings.menuItems.find(
        (i) => i.id === adminSettings.bestMenuItemId
      ) || adminSettings.menuItems[0]
    );
        ...
        {showNotification && (
          ...
          >
            <div className="bg-[#111]/90 backdrop-blur-xl border-l-2 border-[#BF953F] p-5 shadow-2xl flex items-center gap-4">
              <div>
                <p className="text-[#BF953F] text-[9px] font-black uppercase tracking-widest mb-1">
                  {notificationMessage.title}
                </p>
                <p className="text-zinc-300 text-[10px] leading-relaxed font-medium">
          ...
          <div className="flex items-center gap-2 px-3 py-1 bg-white/5 rounded-full border border-white/5 w-fit">
            <div
              className={`w-1.5 h-1.5 rounded-full ${
                tierConfig.color === "#ffffff" ? "bg-white" : "bg-[#BF953F]"
              } animate-pulse`}
            />
      ...
      >
        ...
        <div className="relative z-10 flex flex-col h-full justify-between">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-[8px] text-[#BF953F] font-black uppercase tracking-[0.3em] mb-2 opacity-80">
                Current Cycle
              </p>
              <h2 className="text-4xl font-serif text-white">
                {visits}{" "}
                <span className="text-zinc-700 text-xl font-sans font-light">
                  / {CYCLE_TARGET}
                </span>
              </h2>
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-between items-center text-[9px] font-bold uppercase tracking-[0.2em] text-zinc-500">
              ...
              {isNearCompletion && (
                <span className="text-[#E5C579]">Almost There</span>
              )}
            </div>
          </div>
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <p className="text-[9px] text-zinc-400 tracking-widest uppercase">
              {SOCIAL_PROOF_MESSAGES[proofIndex]}
            </p>
          </motion.div>
        </AnimatePresence>
      </div>
          ...
          >
            ...
            <div className="absolute bottom-0 left-0 p-8 w-full">
              <h4 className="text-3xl font-serif text-white mb-2">
                {highlightedItem.nameEn}
              </h4>
              <p className="text-xs text-zinc-400 line-clamp-2 leading-relaxed mb-4 w-3/4 font-light">
                {highlightedItem.descEn}
              </p>
