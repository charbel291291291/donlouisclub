...
const OffersScreen: React.FC<OffersScreenProps> = ({ adminSettings }) => {
  ...
  const activeOffers = useMemo(() => {
    return adminSettings.offers.filter(o => adminSettings.activeOfferIds.includes(o.id));
  ...
  return (
    ...
    >
      <header className="mb-12">
        ...
        <p className="text-zinc-600 text-[10px] tracking-[0.3em] uppercase">Exclusive Benefits</p>
      </header>
      ...
      <div className="space-y-6">
        {activeOffers.length > 0 ? activeOffers.map((offer, idx) => (
            ...
            <div className="relative z-10">
               <div className="flex justify-between items-start mb-8">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center border ${
                     ...
                     {offer.type === 'latenight' ? <Timer size={20} /> : <Zap size={20} />}
                  </div>
               ...
               <h3 className="text-2xl font-serif text-white mb-2">{offer.titleEn}</h3>
               <p className="text-xs text-zinc-500 font-arabic mb-0 opacity-70">{offer.titleAr}</p>
            </div>

            {/* Hover Glow */}
            <div className={`absolute -bottom-10 -right-10 w-40 h-40 rounded-full blur-[80px] opacity-10 transition-opacity duration-1000 group-hover:opacity-30 ${
          ...
          <div className="text-center py-20 opacity-50">
             ...
             <p className="text-zinc-600 text-xs tracking-widest uppercase">No Active Privileges</p>
          </div>
        )}
      </div>
    </motion.div>
