...
const SectionCard = ({
  ...
  >
    ...
    <div
      className={`w-12 h-12 rounded-full bg-white/5 flex items-center justify-center mb-4 ${color}`}
    >
      <Icon size={24} />
    </div>
    <div>
      ...
      <div className="space-y-2">
        ...
        <button
          ...
          className={`w-full p-3 rounded-lg border text-sm font-medium transition-all ${
            isUploading
              ? "bg-[#BF953F]/20 border-[#BF953F] text-[#BF953F] cursor-not-allowed"
              : "bg-black border-white/10 text-white hover:border-[#BF953F] hover:text-[#BF953F]"
          }`}
        >
          ...
          <div className="space-y-6">
            ...
            {editingItem ? (
              <div className="bg-[#1a1a1a] p-6 rounded-2xl border border-white/10 space-y-4">
                ...
                <textarea
                  ...
                  className="w-full bg-black p-3 rounded-lg border border-white/10 text-white text-sm h-20"
                />

                <div className="flex gap-2 pt-4">
                  <button
                    onClick={() => setEditingItem(null)}
                    className="flex-1 py-3 bg-white/5 rounded-xl text-zinc-400 text-xs font-bold uppercase"
                  >
                    Cancel
                  </button>
                  <button
                    onClick={() => {
                      const newItems = settings.menuItems.some(
                        (i) => i.id === editingItem.id
                      )
                        ? settings.menuItems.map((i) =>
                            i.id === editingItem.id
              ...
              <div className="space-y-2 max-h-[60vh] overflow-y-auto">
                {settings.menuItems.map((item) => (
                  <div
                    key={item.id}
                    className="flex items-center gap-3 p-3 bg-[#0A0A0A] border border-white/5 rounded-xl group"
                  >
                    <img
                      src={item.image}
                      ...
                      <button
                        onClick={() => {
                          if (confirm("Delete item?")) {
                            onUpdateSettings({
                              ...settings,
                              menuItems: settings.menuItems.filter(
                                (i) => i.id !== item.id
                              ),
                            });
                          }
            ...
            {!editingItem && (
              <div className="p-4 bg-[#BF953F]/10 border border-[#BF953F]/20 rounded-xl mt-6">
                <p className="text-[10px] text-[#BF953F] uppercase tracking-widest font-bold mb-2">
                  Active Spotlight
                </p>
                <select
        ...
        return (
          <div className="space-y-6">
            <div className="flex justify-between items-center mb-4">
              ...
              <button
                onClick={() =>
                  setEditingOffer({ id: "o" + Date.now(), type: "weekly" })
                }
                className="flex items-center gap-2 px-4 py-2 bg-white/10 rounded-full text-[10px] font-bold uppercase hover:bg-white/20"
              >
            ...
            {editingOffer ? (
              <div className="bg-[#1a1a1a] p-6 rounded-2xl border border-white/10 space-y-4">
                ...
                />
                <input
                  placeholder="Expiry Text"
                  value={editingOffer.expiry || ""}
                  onChange={(e) =>
                    setEditingOffer({ ...editingOffer, expiry: e.target.value })
                  }
                  className="w-full bg-black p-3 rounded-lg border border-white/10 text-white text-sm"
                />
                <select
                ...
                <div className="flex gap-2 pt-4">
                  ...
                  <button
                    onClick={() => {
                      const newOffers = settings.offers.some(
                        (o) => o.id === editingOffer.id
                      )
                        ? settings.offers.map((o) =>
                            o.id === editingOffer.id
                              ? (editingOffer as Offer)
                              : o
                          )
                      ...
                      <div
                        onClick={() => {
                          const newIds = isActive
                            ? settings.activeOfferIds.filter(
                                (id) => id !== offer.id
                              )
                            : [...settings.activeOfferIds, offer.id];
                          onUpdateSettings({
                            ...settings,
                            activeOfferIds: newIds,
                          });
                        }}
                      ...
                      <button
                        onClick={() => {
                          if (confirm("Delete offer?")) {
                            onUpdateSettings({
                              ...settings,
                              offers: settings.offers.filter(
                                (o) => o.id !== offer.id
                              ),
                              activeOfferIds: settings.activeOfferIds.filter(
                                (id) => id !== offer.id
                              ),
                            });
                          }
                ...
                {customers
                  ...
                  .map((customer) => (
                    <div
                      ...
                      className="flex justify-between items-center p-3 bg-[#0A0A0A] border border-white/5 rounded-xl"
                    >
                      <div>
                        ...
                        <p className="text-zinc-500 text-[10px]">
                          {customer.phone} • {customer.member_id}
                        </p>
                      </div>
                      <div className="flex items-center gap-3">
                        <div className="text-right">
                          <p className="text-[#BF953F] font-bold text-sm">
                            {customer.points} pts
                          </p>
                          <p className="text-zinc-600 text-[9px]">
                            {customer.visits_in_cycle}/5 Cycle
                          </p>
                        </div>
                        <button
                          onClick={() =>
                            handleDeleteCustomer(customer.member_id)
                          }
                          className="p-2 bg-red-900/10 text-red-500 rounded-lg"
                        >
                          <Trash2 size={14} />
                        </button>
                      </div>
                ...
                {[
                  ...
                  <button
                    ...
                    onClick={() =>
                      onUpdateSettings({
                        ...settings,
                        brand: { ...settings.brand, primaryColor: color },
                      })
                    }
                    className={`h-12 rounded-xl border-2 transition-transform ${
                      settings.brand.primaryColor === color
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  onClick={() =>
                    onUpdateSettings({
                      ...settings,
                      brand: { ...settings.brand, bgTone: "#030303" },
                    })
                  }
                  className={`p-4 rounded-xl border ${
                    settings.brand.bgTone === "#030303"
                      ? "bg-[#030303] border-[#BF953F]"
                ...
                <button
                  onClick={() =>
                    onUpdateSettings({
                      ...settings,
                      brand: { ...settings.brand, bgTone: "#0F172A" },
                    })
                  }
                  className={`p-4 rounded-xl border ${
                    settings.brand.bgTone === "#0F172A"
                      ? "bg-[#0F172A] border-[#BF953F]"
          ...
          <div className="space-y-6">
              ...
              <div className="mb-4">
                <Toggle
                  ...
                  onChange={(v: boolean) =>
                    onUpdateSettings({
                      ...settings,
                      splash: { ...settings.splash, animate: v },
                    })
                  }
                />
              </div>
                ...
                {["fade", "glow", "cinematic"].map((style: any) => (
                  <button
                    ...
                    onClick={() =>
                      onUpdateSettings({
                        ...settings,
                        splash: { ...settings.splash, animationStyle: style },
                      })
                    }
                    className={`p-4 rounded-xl border flex items-center justify-between ${
                      settings.splash.animationStyle === style
                        ? "bg-white/10 border-[#BF953F] text-[#BF953F]"
            ...
            <div className="group">
              ...
              <FileUploadButton
                ...
                currentImage={settings.splash.backgroundImage}
            ...
            <div className="p-6 bg-[#0A0A0A] rounded-2xl border border-red-900/20 flex flex-col items-center text-center">
              ...
              <button
                onClick={() => {
                  ...
                  if (n && n.length === 4)
                    onUpdateSettings({ ...settings, cashierPin: n });
                }}
      ...
      <div className="sticky top-0 z-50 bg-black/80 backdrop-blur-xl border-b border-white/5 px-6 pt-12 pb-4 flex items-center justify-between">
        {activeSection ? (
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-[#BF953F] text-[9px] uppercase tracking-widest font-black mb-1">
                {BRAND_INFO.fullName.toUpperCase()}
              </p>
        ...
        {!activeSection && (
          <button
            onClick={onExit}
            className="px-4 py-2 bg-white/5 rounded-full text-xs font-bold text-zinc-400"
          >
            Exit
          </button>
        )}
